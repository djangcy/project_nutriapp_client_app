...
class _ViewContentState extends State<_ViewContent> {
  ...
  Widget build(BuildContext context) {
    ...
    final BorderSide? effectiveSide =
        widget.viewSide ?? widget.viewTheme.side ?? widget.viewDefaults.side;
...
/// * [SearchBarTheme], a widget that overrides the default configuration of a search bar.
/// * [SearchViewTheme], a widget that overrides the default configuration of a search view.
class MultilineSearchBar extends StatefulWidget {
  /// Creates a Material Design search bar.
  const MultilineSearchBar({
    super.key,
    this.controller,
    this.focusNode,
    this.hintText,
  ...
  /// If the text is written in a single line the minimum height will be 1.
  /// Defaults to 1.
  final int minLines;

  /// The maximum number of lines to occupy when the content spans fewer lines.
  ///
  /// Defaults to 1.
  final int maxLines;
  ...
  @override
  State<MultilineSearchBar> createState() => _MultilineSearchBarState();
}

class _MultilineSearchBarState extends State<MultilineSearchBar> {
